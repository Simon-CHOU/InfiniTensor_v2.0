...
def test_lpnorm(runtime, torch_rng_seed, p, dim, keepdim):
    print(f"Testing LpNorm p={p}, dim={dim}, keepdim={keepdim} with runtime on device: {runtime}")
    ...
    translator.import_from_fx(model, [input_tensor], is_real_tensor=True, dynamic_shapes=False)
