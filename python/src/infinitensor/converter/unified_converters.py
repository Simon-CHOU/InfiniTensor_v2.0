...
def convert_log_softmax(translator, node):
    ...
    if dim != rank - 1:
        ...
        translator.tensors[node] = translator.builder.transpose(output_transposed, inv_perm, None)
    else:
        translator.tensors[node] = translator.builder.log_softmax(input_tensor, dim, None)
...
def convert_rms_norm(translator, node):
    ...
    translator.tensors[node] = translator.builder.rms_norm(input_tensor, weight_tensor, float(eps), None)
...
def convert_linalg_vector_norm(translator, node):
    ...
    translator.tensors[node] = translator.builder.lp_norm(input_tensor, p_val, dims, keepdim, None)
